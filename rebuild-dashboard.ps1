...
# ── Paths ─────────────────────────────────────────────────────────────────────
$P11_DUMP_DIR = "C:\Users\MuruganVenugopal\.bob\playground\.bob\tmp\xlsx-dumps\Brands_Installs Data\.bob\tmp\xlsx-dumps\P11 - 7th Aug-863ad975e4a47e79"
$OUT_DIR      = "C:\Users\MuruganVenugopal\.bob\playground\.bob\tmp\xlsx-dumps\Brands_Installs Data"

# ── Load both sheets ──────────────────────────────────────────────────────────
$essSheet = Get-Content "$P11_DUMP_DIR\ESS_Installs.json"      -Raw | ConvertFrom-Json
$svlSheet = Get-Content "$P11_DUMP_DIR\SNs_Config____SVL.json" -Raw | ConvertFrom-Json
...
$v1Json   | Set-Content "$OUT_DIR\v1.json"    -Encoding UTF8 -NoNewline
$v2Json   | Set-Content "$OUT_DIR\v2.json"    -Encoding UTF8 -NoNewline
$v3Json   | Set-Content "$OUT_DIR\v3.json"    -Encoding UTF8 -NoNewline
$rowsJson | Set-Content "$OUT_DIR\rows2.json" -Encoding UTF8 -NoNewline

Write-Host "All P11 data files written."
